...
Chart = (function () {
    ...
    function update() {
        ...
        let maxData = d3.max(_data, function (d) {
            return d.overdue + d.hold + d.paid
        });
        ...
        let line = d3.svg.line()
            ...
            .y(function (d) {
                return (_y(d.overdue + d.hold + d.paid))
            });
        ...
        _chartD3.selectAll("g.tooltip").data(_data).transition().duration(600).ease("quad-in-out")
            .attr("transform", function (d, i) {
                ...
                return "translate(" + anchroX + ",-" + ((_chartSize.height - _y(d.overdue + d.hold + d.paid)) + 12) + ")";
            });


        format = getLocale().numberFormat("n");
        _chartD3.selectAll("text.totale").data(_data).text(function (d) {
            return '€ ' + format(d3.format(".2f")(d.overdue + d.hold + d.paid))
        });
        _chartD3.selectAll("text.attesa").data(_data).text(function (d) {
            return '€ ' + format(d3.format(".2f")(d.hold))
        });
        _chartD3.selectAll("text.pagate").data(_data).text(function (d) {
            return '€ ' + format(d3.format(".2f")(d.paid))
        });
        _chartD3.selectAll("text.ritardo").data(_data).text(function (d) {
            return '€ ' + format(d3.format(".2f")(d.overdue))
        });
        ...
        _chartD3.selectAll("rect.attesa").data(_data).transition().duration(600)
            .ease("quad-in-out")
            .attr("y", function (d) {
                return -(_chartSize.height - _y(d.hold))
        ...
        _chartD3.selectAll("rect.pagate").data(_data).transition().duration(600)
            .ease("quad-in-out")
            .attr("y", function (d) {
                return -(_chartSize.height - _y(d.hold + d.paid))
            }).attr("height", function (d) {
            return _chartSize.height - _y(d.paid)
        });
        ...
        _chartD3.selectAll("rect.ritardo").data(_data).transition().duration(600)
            ...
            return _chartSize.height - _y(d.overdue)
        });
    }
    ...
    function build() {
        ...
        _y = d3.scale.linear().domain([0, d3.max(_data, function (d) {
            return d.overdue + d.hold + d.paid
        })])
        ...
        let singleBars = bars.append("g")
            .attr("class", "status-bar")
            .attr("transform", 'translate(-3,0) scale(1,0)')

        singleBars.append("rect").attr("class", "attesa").attr("width", 6).attr("fill", '#999999').attr("y", function (d) {
            return -(_chartSize.height - _y(d.hold))
        }).attr("height", function (d) {
            return _chartSize.height - _y(d.hold)
        })
        singleBars.append("rect").attr("class", "pagate").attr("width", 6).attr("fill", '#006693').attr("y", function (d) {
            return -(_chartSize.height - _y(d.hold + d.paid))
        }).attr("height", function (d) {
            return _chartSize.height - _y(d.paid)
        })
        singleBars.append("rect").attr("class", "ritardo").attr("width", 6).attr("fill", '#a42c0f').attr("y", function (d) {
            return -(_chartSize.height - _y(d.overdue + d.hold + d.paid))
        }).attr("height", function (d) {
            return _chartSize.height - _y(d.overdue)
        })
        ...
        let tooltip = l2.selectAll("g.tooltip")
            .data(_data);
        ...
        let tooltips = tooltip.append("g").attr("class", "tooltip").attr("transform", function (d, i) {
        ...
        tooltips.append("text").attr("class", "t totale").attr("transform", "translate(70,15)").attr("text-anchor", "end").text(function (d) {
            return '€ ' + format(d.overdue + d.hold + d.paid)
        });
        tooltips.append("text").attr("class", "t attesa").attr("transform", "translate(130,15)").attr("text-anchor", "end").text(function (d) {
            return '€ ' + format(d.hold)
        });
        tooltips.append("text").attr("class", "t pagate").attr("transform", "translate(190,15)").attr("text-anchor", "end").text(function (d) {
            return '€ ' + format(d.paid)
        });
        tooltips.append("text").attr("class", "t ritardo").attr("transform", "translate(250,15)").attr("text-anchor", "end").text(function (d) {
            return '€ ' + format(d.overdue)
        });
